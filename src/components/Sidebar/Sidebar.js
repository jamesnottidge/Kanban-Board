...
export const Sidebar = (props) => {
  ...
  return (
    <div className=" sidebar text-left ">
      ...
      <div className="sidebar-content font-bold  text-slate-600">
        <p className="px-4 font-bold  text-slate-600 mb-4">
        ...
        {boardsList.map((board) => {
          return (
            <div
              ...
              className={`flex py-2 rounded-r-full active:bg-cyan-700 ${
                board.name === currentBoard.name ? "color text-white" : null
              }`}
            >
              <img src={Boardsvg} alt="sdcsd" className="w-8 ml-4 "></img>

              <span className="ml-3">{board.name}</span>
            </div>
          );
        })}
        <div
          onClick={() => setShowAddBoardModal(true)}
          className="create-board mt-4 flex"
        >
          <img src={Boardsvg} alt="sdcsd" className="w-8 ml-4 "></img>+ Create
          New Board
        </div>
      </div>
    </div>
